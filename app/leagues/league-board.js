...
export default function LeagueBoard() {
  ...
  useEffect(() => {
    supabase.rpc("get_my_league_info").then(({ data }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    supabase
      .rpc("get_league_standings", { p_school_id: myInfo.school_id, p_league: active })
      .then(({ data }) => {
        setRows(data || []);
        setLoading(false);
      });
  ...
  return (
    <div>
      ...
      {loading && <p>Loading...</p>}
      {!loading && rows.length === 0 && <p>No students in this league yet.</p>}
