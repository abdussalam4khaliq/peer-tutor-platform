...
export default function TutorLeaderboardTabs({ departmentId, facultyId, schoolId }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      supabase.rpc("get_tutor_leaderboard", { p_scope: scope.key, p_scope_id: scope.scopeId, p_limit: 50 }),
      supabase.rpc("get_my_tutor_rank", { p_scope: scope.key, p_scope_id: scope.scopeId }),
  ...
  return (
    <div>
      <div className="toggle" style={{ marginBottom: 20, flexWrap: "wrap" }}>
        {scopes.map((s) => (
          <button key={s.key} type="button" className="toggle__btn" data-active={active === s.key} onClick={() => setActive(s.key)}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      {loading && <p>Loading...</p>}
      {!loading && rows.length === 0 && <p>No ranked tutors in this scope yet.</p>}
